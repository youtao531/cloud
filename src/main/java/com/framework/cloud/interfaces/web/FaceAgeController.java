...
@RestController
@ApiSupport(order = 1)
@Tag(name = "年龄检测", description = "年龄检测")
@RequestMapping(value = "/api/faces/age", produces = MediaType.APPLICATION_JSON_VALUE)
public class FaceAgeController {
    ...
    @Operation(summary = "年龄检测", description = "年龄检测")
    @Parameters(value = {@Parameter(name = "faceImage", description = "人脸照片", in = ParameterIn.QUERY)})
    public ComResult<Integer> agePredictor(MultipartFile faceImage) {
        log.info("年龄检测 开始");
        Assert.notNull(faceImage, "上传人脸照片不能为空");
        Integer result = faceAgeService.agePredictor(faceImage);
        Map<String, Object> imageInfo = MultipartFileHelper.imageInfo(faceImage);
        log.info("年龄检测 结果 {}={}", imageInfo, result);
